...
function createUserDocument({ uid, password, firstName, lastName, description, profilePicture, availability }) {
    // ============= validate inputs =============

    uid = validation.validateUserId(uid);
    password = validation.validateAndTrimString(password, "Password"); // should be hashed already
    firstName = validation.sanitizeSpaces(validation.validateAlphabetical(firstName, "First Name", 1));
    lastName = validation.sanitizeSpaces(validation.validateAlphabetical(lastName, "Last Name", 1));
    description = validation.validateAndTrimString(description, "Description", true);
    availability = validation.validateArrayElements(availability, "Availability", (timeslot) => timeslot, 7); // todo MG - validate Timeslot objects
...
async function createUser({ uid, password, firstName, lastName, description, profilePicture, availability }) {
...
// remove the user with the specified ID from the DB
...
// todo MG - maybe use PATCH instead of POST? could add logic to only modify the fields that are not undefined
// update ALL the data of the user with the specified ID, and return the updated user
async function updateUser({ uid, password, firstName, lastName, description, profilePicture, availability }) {
    uid = validation.validateUserId(uid);
    const collection = await usersCollection();
    const newUser = createUserDocument({ uid, password, firstName, lastName, description, profilePicture, availability });

    // copy `meetings` from old object and copy it over to the new one
    try {
        const existing = await getUserById(uid);
        newUser.meetings = existing.meetings;
    } catch (err) {
        // user not found, so can't copy fields
    }

    const updated = await collection.findOneAndUpdate({ _id: uid }, { $set: newUser }, { returnDocument: "after" });
...
async function modifyUserMeeting(uid, meetingId, isAdd) {
    uid = validation.validateUserId(uid);
...
export { createUserDocument, createUser, getAllUsers, getAllUserIDs, getUserById, deleteUser, updateUser, modifyUserMeeting };
