...
export async function createComment({ uid, meetingId, body }) {
    //TODO BL: Once meeetings exist, good idea to query meetings DB if meetingId is a real ID
    //Throw if it isn't.
...
export async function updateComment(id, newBody) {
    id = validateStrAsObjectId(id, "Comment ID");
    newBody = validateAndTrimString(newBody, "Comment Text", false);
    let timestamp = new Date();

    const collection = await commentsCollection();
    const updated = await collection.findOneAndUpdate({ _id: new ObjectId(id) }, { $set: { body: newBody, dateUpdated: timestamp } }, { returnDocument: "after" });
